...
class Order extends Model implements JsonSerializable
{
	...
	{
		global $db, $tx;
		$db->query("insert into {$tx}orders(customer_id,quantity,order_date,status,tracking_id,total_amount,created_at,updated_at,delivery_date,shipping_address,paid_amount,discount)values('$this->customer_id','$this->quantity','$this->order_date','$this->status','$this->tracking_id','$this->total_amount','$this->created_at','$this->updated_at','$this->delivery_date','$this->shipping_address','$this->paid_amount','$this->discount')");
		return $db->insert_id;
	}
...
";
	...
	static function html_table($page = 1, $perpage = 10, $criteria = "", $action = true)
	{
		...
		if ($action) {
			$html .= "<tr><th>Id</th><th>Customer Id</th><th>Quantity</th><th>Order Date</th><th>Status</th><th>Tracking Id</th><th>Total Amount</th><th>Created At</th><th>Updated At</th><th>Delivery Date</th><th>Shipping Address</th><th>Paid Amount</th><th>Discount</th><th>Action</th></tr>";
		} else {
			$html .= "<tr><th>Id</th><th>Customer Id</th><th>Quantity</th><th>Order Date</th><th>Status</th><th>Tracking Id</th><th>Total Amount</th><th>Created At</th><th>Updated At</th><th>Delivery Date</th><th>Shipping Address</th><th>Paid Amount</th><th>Discount</th></tr>";
		}
		while ($order = $result->fetch_object()) {
			$action_buttons = "";
			if ($action) {
			...
			$html .= "<tr><td>$order->id</td><td>$order->customer_id</td><td>$order->quantity</td><td>$order->order_date</td><td>$order->status</td><td>$order->tracking_id</td><td>$order->total_amount</td><td>$order->created_at</td><td>$order->updated_at</td><td>$order->delivery_date</td><td>$order->shipping_address</td><td>$order->paid_amount</td><td>$order->discount</td> $action_buttons</tr>";
		}
		$html .= "</table>";
...
ORDER BY o.id, od.id;");
		...
		$html .= "<tr><th>Quantity</th><td>$order->quantity</td></tr>";
