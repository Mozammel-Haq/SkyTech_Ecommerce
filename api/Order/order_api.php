...
class OrderApi
{
	...
	function save_order($data)
	{
		...
		$order->shipping_address = $data["shipping_address"];
		$order->paid_amount = $data["paid_amount"] ?? null;
		...
		foreach($products as $data){
			...
			$orderdetail->variation_id = $data["variation_id"] ?? null;
			...
			$inventory->warehouse_id = $data["warehouse_id"]?? null;
			$inventory->supplier_id = $data["supplier_id"]?? null;
			$inventory->quantity = $data["qty"] * -1;
			$inventory->created_at = $now;
			$inventory->updated_at = $now;
			$inventory->transaction_type_id = $data["transaction_type_id"] ?? null;
